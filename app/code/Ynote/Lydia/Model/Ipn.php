...
/**
 * Lydia Instant Payment Notification processor model
 */
class Ipn
{
    protected $_api = null;
    ...
    /**
     * @var \Ynote\Lydia\Model\Method\AbstractMeans
     */
     ...
     *
     * @param string $data
     * @param \Ynote\Lydia\Model\Method\AbstractMeans $methodInstance
     */
    ...
    protected function _checkIpAddresses()
    {
        if ($this->_config->getCheckByIpAddress()) {
            $ipAdresses = $this->_response['lydiaserver_ip_adresses'];
    ...
    protected function _processOrderPayment()
    {
        try {
            ...
            if (!$this->_order->isCanceled()) {
                ...
                if ($this->_response['hash']['capture_mode'] == \Ynote\Lydia\Model\Config::PAYMENT_ACTION_CAPTURE ||
                        $this->_response['hash']['capture_mode'] == \Ynote\Lydia\Model\Config::PAYMENT_ACTION_AUTHORIZE) {
    ...
    {
        $message = __('Payment rejected by Lydia') . '<br /><br />' . $this->_api->describeResponse($this->_response['hash']);
        $hasError = true;
